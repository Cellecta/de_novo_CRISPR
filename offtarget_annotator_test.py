...
def summarize_results_by_guide(results, summary_output=None):
    ...
    for guide_seq, alignments in results.items():
        ...
        avg_cfd_cds = round(calcMitGuideScore(sum(cfd_cds)) if cfd_cds else 0.0)
...
def main(sam_file, genome_fasta, gtf_path, output_json):
    ...
    print(f"[SUCCESS] Off-target annotations saved to {output_json}")

    # Per-guide summary
    summarize_results_by_guide(
        results, summary_output=output_json.replace(".json", "_guide_summary.csv")
    )
    print("[INFO] Guide-level summary saved.")

# ---- CLI ----

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--sam", required=True, help="Full SAM line with XA:Z: field")
    ...
    main(args.sam, args.genome, args.gtf, args.out)
